...
import { importRegex, scriptClientRE, scriptRE, scriptRegex, scriptSetupRE } from '../constant'
  ...
  const addRenderRule = (type: string) => {
    ...
    md.renderer.rules[type] = (tokens, idx, options, env, self) => {
      ...
      if (demoImportCodeStr) {
        ...
        const { importRecord: statementObj, directImportRecord } = importStatementObj(_code)
        const importStatementCode = buildImportStatement(statementObj).join(os.EOL).trim()
        const finalCode = [importStatementCode, ...directImportRecord].join(os.EOL).trim()
        ...
        if (existingSetupScriptIndex > -1) {
          tags[existingSetupScriptIndex].content = `<script lang="ts" setup >${finalCode}</script>`.trim()
        } else {
          tags.unshift({
...
const liveEditorTemplate = ({ sourceCode, hideCode, noStyle, scope }: ILiveEditor) => {
  return `<LiveEditor :scope="{ ${scope} }" sourceCode="${sourceCode}" :hideCode="${hideCode}" :noStyle="${noStyle}" ></LiveEditor>`
}

export const parseProps = <T extends Record<string, any> = any>(content: string) => {
